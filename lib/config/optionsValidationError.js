...
var getSchemaPartText = function getSchemaPartText(schemaPart, additionalPath) {
  ...
  }var schemaText = OptionsValidationError.formatSchema(schemaPart); // eslint-disable-line
...
var OptionsValidationError = function (_Error) {
  _inherits(OptionsValidationError, _Error);

  function OptionsValidationError(validationErrors) {
    _classCallCheck(this, OptionsValidationError);

    var _this = _possibleConstructorReturn(this, (OptionsValidationError.__proto__ || Object.getPrototypeOf(OptionsValidationError)).call(this));

    if (Error.hasOwnProperty('captureStackTrace')) {
      // eslint-disable-line
      Error.captureStackTrace(_this, _this.constructor);
    }
    _this.name = 'PageSkeletonOptionsValidationError';

    _this.message = '' + ('Invalid configuration object. ' + 'page-skeleton-webpack-plugin has been initialised using a configuration object that does not match the API schema.\n') + validationErrors.map(function (err) {
  ...
  _createClass(OptionsValidationError, null, [{
    key: 'formatSchema',
    ...
    value: function formatValidationError(err) {
      ...
      if (err.keyword === 'additionalProperties') {
        ...
        if (err.children && err.children.length > 0) {
          return dataPath + ' should be one of these:\n' + getSchemaPartText(err.parentSchema) + '\n' + ('Details:\n' + err.children.map(function (e) {
            return ' * ' + indent(OptionsValidationError.formatValidationError(e), '   ', false);
          }).join('\n'));
        }
        return dataPath + ' should be one of these:\n' + getSchemaPartText(err.parentSchema);
